...
site_inf      <- site_inf[site_inf$FCA!=2,]
...
substr(WHX[c(Model+1)],72,76) <- 'WHAPS'
...
for (i in 1: nrow(site_inf)) {
  ...
  if(site_inf$FCA[i]==1){
    substr(WHX[CULID+2],7, 22) <- '940002 Lingfen_crp'
  ...
  if(site_inf$FCA[i]==2){
    substr(WHX[CULID+2],7, 22) <- '940000 Xiayan22_crp'
  ...
  if(site_inf$FCA[i]==3){
    substr(WHX[CULID+2],7, 22) <- '940001 Changhan_crp'
  ...
  write(SMout, file = paste('E:/Researchs/Papers/Wheat climate suatiable/SIMOUTS/Nwheat/Baseline/', 
  ...
  for (n in YEAR[2] : max(YEAR)) {
    ...
    ##@@
    system("C:/DSSAT47/DSCSM047.EXE WHAPS047 B DSSBatch.v47")
    ...
    if('Evaluate.OUT'%in%list.files(Run_files)&'Summary.OUT'%in%list.files(Run_files)){
      ...
      flowering <- substr(EvaluateOut[4], 33, 35)
      maturity  <- substr(EvaluateOut[4], 81, 83)
      yield     <- substr(EvaluateOut[4], 95, 99)
    ...
    write(SMDATA, file = paste('E:/Researchs/Papers/Wheat climate suatiable/SIMOUTS/Nwheat/Baseline/', 
